...
# Usage: harness review [project_dir]
...
LATEST_HANDOFF=$(ls -t "${HARNESS_DIR}/handoffs/"handoff_*.md 2>/dev/null | head -1)

if [[ -z "$LATEST_HANDOFF" ]]; then
    echo "Warning: No handoff files found in ${HARNESS_DIR}/handoffs/"
    ...
    HANDOFF_INSTRUCTION="Read the latest handoff file: .harness/handoffs/${HANDOFF_NAME}"
fi

PROMPT="You are the Reviewer agent in a dual-agent workflow.

START by reading these files in order:
1. .harness/roles/reviewer-brief.md
2. ${HANDOFF_INSTRUCTION}
3. .harness/quality/review-checklist.md
...
- Generate a review file in .harness/reviews/ following .harness/quality/output-format.md
